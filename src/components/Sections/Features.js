...
const FeaturesSection = (props) => {
  const {
    section: { section_id, background, title, subtitle, features },
    annotationPrefix,
  } = props;
  ...
  return (
    <section
      id={section_id}
      className={`block features-block bg-${background} outer`}
      {...toFieldPath(annotationPrefix)}
    >
      <div className="block-header inner-small">
        {title && (
          <h2 className="block-title" {...toFieldPath('.title')}>
            {title}
          </h2>
        )}
        {subtitle && (
          ...
          {features.map((feature, featureIndex) => {
            ...
            return (
              <div
                ...
                {...toFieldPath(`.features.${featureIndex}`)}
              >
                <div className="grid">
                  {image && (
